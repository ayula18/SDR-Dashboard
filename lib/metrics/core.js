 ...
 * it came in, as in dash_v_linkedin_leads.
 */
export async function recentReplies(filters, { limit = 30 } = {}) {
  const p = new Params();
  return qp(`
    SELECT * FROM (
      SELECT v.replied_at AS "repliedAt", v.platform, v.verdict, v.is_negative AS negative, v.is_auto AS auto,
             v.quote, v.reason, v.classifier, v.company_domain AS domain, v.person_name AS name, c.name AS campaign, c.sdr
        FROM dash_v_reply_verdicts v
        JOIN dash_v_campaigns c ON c.external_id = v.campaign_external_id AND c.platform = 'instantly'
       WHERE v.platform = 'email' AND ${scope('campaign', 'c', filters, p)} AND NOT v.is_auto
      UNION ALL
      SELECT v.replied_at, v.platform, v.verdict, v.is_negative, v.is_auto,
             v.quote, v.reason, v.classifier, COALESCE(a.company_domain, v.company_domain), v.person_name, c.name, c.sdr
        FROM dash_v_reply_verdicts v
        JOIN LATERAL (
          SELECT l.campaign_id, l.company_domain
        ...
        JOIN dash_v_campaigns c ON c.id = a.campaign_id
       WHERE v.platform = 'linkedin' AND ${scope('campaign', 'c', filters, p)} AND NOT v.is_auto
    ) replies
