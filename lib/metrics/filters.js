...
export function parseFilters(searchParams) {
  ...
  // Filters that accept several values at once arrive comma-separated: "Akhil,Dheeraj".
  const pickList = key => {
    const value = pick(key);
    const items = value ? [...new Set(value.split(',').map(v => v.trim()).filter(Boolean))] : [];
...
export function describeRange(f) {
  return {
    preset: f.preset,
    // The trend charts' grain (the weekly/monthly switch), so a page labels its charts by what the data was built with.
    grain: f.trendGrain || f.grain,
    // The whole weeks or months the range itself is counted in; custom dates round out to these.
    rangeGrain: f.grain,
    from: f.from,
    to: f.to,
