...
export async function getLeaderboard(filters) {
  // Picking a few SDRs narrows who is on the board, not the maths: each row is still that SDR's own work.
  const picked = filters.sdr ? new Set(Array.isArray(filters.sdr) ? filters.sdr : [filters.sdr]) : null;
  ...
  const sdrs = team.filter(t => !picked || picked.has(t.name)).map(t => row(t.name, t.color));
  const unattributed = picked ? null : row(null, null);
  ...
  return {
    ...
    unattributed: unattributed && unattributed.sent + unattributed.replies + unattributed.meetings > 0 ? unattributed : null,
  };
}
