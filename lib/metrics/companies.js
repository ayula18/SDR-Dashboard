...
/**
 * One row per company the filtered outreach touched on either channel since
 * `since`: email leads and LinkedIn people side by side, the best reply on
 * either, and meetings after the first touch. A company LinkedIn could not
 * match to a domain is listed by its LinkedIn name.
 */
export async function companiesAcrossChannels(filters, { since = null, limit = 5000 } = {}) {
  const p = new Params();
  const rank = p.add(REPLY_RANK);
  const emailWhere = [scope('lead', 'l', filters, p), 'l.company_domain IS NOT NULL'];
  if (since) emailWhere.push(`l.created_at_src >= ${p.add(since)}`);
  const linkedinWhere = [scope('linkedin', 'l', filters, p), LI_IS_COMPANY];
  if (since) linkedinWhere.push(`l.added_at >= ${p.add(since)}`);
  ...
  const rows = await qp(`
    ...
    merged AS (
      SELECT COALESCE(e.company, li.company) AS company,
             ...
             e.sdrs AS e_sdrs, e.campaigns AS e_campaigns, e.people AS e_people, e.contacted AS e_contacted,
    ...
    SELECT m.*, co.company_name AS matched_name, ${COMPANY_TYPE} AS company_type, ${EMPLOYEES} AS employees,
           co.icp_decision AS icp, co.category, mt.meetings, mt.held, mt.qualified
      FROM merged m
  ...
  return rows
    .map(r => {
      const email = r.e_people ? {
        campaigns: r.e_campaigns, people: r.e_people, contacted: n(r.e_contacted),
      ...
      return {
        ...
        stage: companyStage({
          ...
          contacted: n(email?.contacted) + n(linkedin?.invited) + n(linkedin?.messaged),
        }),
...
/** Headline counts for a list from companiesAcrossChannels. */
