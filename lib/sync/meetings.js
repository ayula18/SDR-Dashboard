...
/**
 * Replaces dash_meetings with the contents of a "Qualified Meetings 2026 -
 * Happened Audit" CSV export. The sheet is the source of truth, so a full
 * replace (in one transaction) is simpler and safer than diffing.
 */
export async function importMeetingsCsv(text, { sourceFile = 'upload' } = {}) {
  ...
  const rows = records.map(r => {
    ...
    return [
      ...
      r['Confidence'] || null, sourceFile, importedAt,
    ];
  });

  const written = await withTx(async client => {
    await client.query('DELETE FROM dash_meetings');
    return upsertRows('dash_meetings', COLUMNS, rows, { conflict: ['id'], client });
  });
